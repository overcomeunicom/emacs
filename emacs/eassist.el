...
(defvar eassist-prefix-path '("inc" "include" "src" "source")
"This variable defines possible prefix paths that hold the include headers or source files.
Presume that the header files are placed in inc or include directory,
 and c/cpp files are placed in src or source directory.")


(defun eassist-extract-base-directory ()
 "Extract base directory exclude the eassist-prefix-path and file name.
 for example, from ~/workspace/project1/src/abc.cpp, we will extract  ~/workspace/project1 ,
 and for ~/workspace/project1/abc.cpp, we will get nil instead"
 (interactive)
 (let* ((path-element (split-string (buffer-file-name) "/"))
        (path-element-length (length path-element))
        (prefix-path-count (length eassist-prefix-path))
        (value)
        (inner-index 0)
        (index 0))
   (progn
     (while (< index prefix-path-count)
       (if (string= (nth (- path-element-length 2) path-element) (nth index eassist-prefix-path))
           (setq value (progn
                         (while (< inner-index (- path-element-length 2))
                           (progn
                             (setq value (concat  value "/" (nth inner-index path-element)))
                             (setq inner-index (1+ inner-index))
                             )
                           )
                         value
                         )
...
for example *.hpp <--> *.cpp."
  ...
  (let* ((ext (file-name-extension (buffer-file-name)))
         ...
         (prefix-path (eassist-extract-base-directory))
         (possible-prefix-path eassist-prefix-path)
         (index 0)
         (inner-index 0)
     ...
     (count-ext
      (unless
          (or
           ...
           (while (< index (length possible-prefix-path))
             (setq construct-possible-path (concat prefix-path "/"  (nth index possible-prefix-path) "/"))
             (message "construct-possible-path = %s" construct-possible-path)
             (if (file-directory-p construct-possible-path)
                 (progn
                   (setq inner-index 0)
                   (while (< inner-index (length count-ext))
                     (progn
                       (setq construct-possible-file-name (concat construct-possible-path base-name (nth inner-index count-ext)))
                       (if (file-exists-p construct-possible-file-name)
                           (find-file construct-possible-file-name)
                           )
                       (setq inner-index (1+ inner-index))
                       )
                     )
           ...
           (message "There is no corresponding pair (header or body) file. index = %d, length = %d" index (length possible-prefix-path))))
